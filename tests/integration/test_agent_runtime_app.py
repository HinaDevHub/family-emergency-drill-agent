...
# These tests require a live GCP project + Vertex AI credentials.
# Skip automatically in local/non-cloud environments.
GCP_AVAILABLE = os.environ.get("GOOGLE_CLOUD_PROJECT") is not None
requires_gcp = pytest.mark.skipif(
    not GCP_AVAILABLE,
    reason="GCP credentials not configured — set GOOGLE_CLOUD_PROJECT to run"
)
if GCP_AVAILABLE:
    from google.adk.events.event import Event
    from app.agent_runtime_app import AgentEngineApp
else:
    # Placeholders so type annotations below don't break at collection time
    Event = None  # type: ignore[assignment,misc]
    AgentEngineApp = None  # type: ignore[assignment,misc]
